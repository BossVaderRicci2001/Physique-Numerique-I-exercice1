...
//ENERGIE MECANIQUE ET MOMENT MAGNETIQUE
  ...
  */  
  void printOut(bool write)
  {
    ...
    if((!write && last>=sampling) || (write && last!=1))
    {
      ...
      double magneticMoment   = mass*(v[0]*v[0]+v[1]*v[1])/(2.0*By()); // TODO calculer le moment magnetique 
...
protected:
  ...
  double charge0mass; // charge / mass
...
//ACCELERATION 
  ...
  */
  valarray<double> acceleration(valarray<double>& a) const
  {
    ...
    a[0] = (-charge0mass)*B0*v[1]; // TODO
    a[1] = (charge0mass)*(v[0]*B0 + E0); // TODO
    return a;
  }
...
public:
  ...
  */
  Engine(ConfigFile configFile)
  {
    ...
    charge0mass = charge / mass; // coefficient charge/mass
...
//EULER EXPLICITE
  ...
  */
  ...
  {
    valarray<double> a=valarray<double>(0.e0,2);
    //on utilsie acceleration deja definie pour la modifier en fonction de v  
    a=acceleration(a);
    x += v*dt; // TODO 
    v += a*dt; // TODO 
    t += dt; // mis a jour du temps
...
//RUNGE-KUTTA
  ...
  */
  void step()
  {
    ...
    a=acceleration(a); // definir l'acceleration
    //j'enregiste mon ancienne y
    valarray<double> xold = valarray<double> (x);
    valarray<double> vold = valarray<double> (v);
    // premier demi pas (sondage)
    x += 0.5*v*dt; // TODO
    v += 0.5*a*dt; // TODO
    // on veut l'acceleration en t+dt/2
    a=acceleration(a);
    // je fais mon pas
    x=xold + v*dt;
    v = vold + a*dt;
...
//BORIS BUNEMAN
  ...
  */
  ...
  void step()
  {
   ...
   double omega_c = charge0mass*B0;
   double beta = omega_c*dt;
   valarray<double> x_= x + 0.5*v*dt;
   valarray<double> v_= v + 0.5*charge0mass*E0*dt;
   valarray<double> v_plus = valarray<double> (0.0,2);
   v_plus[0] = (1-pow(beta,2)/4.0)*v_[0] - (beta*v_[1])/(1+pow(beta,2)/4.0);
   v_plus[1] = (1-pow(beta,2)/4.0)*v_[0] - (beta*v_[1])/(1+pow(beta,2)/4.0); 
   v += v_plus + 0.5*charge0mass*E0*dt; // TODO
   x += x_ + 0.5*v*dt; // TODO
   t += dt; // mis a jour du temps
